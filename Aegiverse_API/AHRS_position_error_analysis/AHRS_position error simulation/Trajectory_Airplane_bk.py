...
Theta_Rotation_In_degree = -112.894
...
if __name__ == "__main__":
    ...
    for color, file_path, theta in zip(color_list, file_path_list, theta_list):
        ...
        # original_distance = np.linalg.norm(np.array(coordinates[-1]) - np.array(coordinates[0]))
        # print(coordinates[0])
        # print(coordinates[-1])
        # print(f"原始距離: {original_distance}")

        c0 = coordinates[0]
        coordinates = (R @ (np.array(coordinates) - c0).T).T + c0

        # rotated_distance = np.linalg.norm(np.array(coordinates[-1]) - np.array(coordinates[0]))
        # print(coordinates[0])
        # print(coordinates[-1])
        # print(f"旋轉後距離: {original_distance}")
