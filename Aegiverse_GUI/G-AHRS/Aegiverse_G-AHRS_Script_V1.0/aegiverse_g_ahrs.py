...
SIZE_4 = 4
PACKET_PAYLOAD_LEN = 49             # payload(含 CRC 4B)
...
def parse_packet(pkt: bytes):
    ...
    gps_status = pkt[POS_GPS_STATUS]

    return tm, wx, wy, wz, ax, ay, az, pd, pt, rl, yw, gps_status
...
def print_csv(t, wx, wy, wz, ax, ay, az, pd, pt, rl, yw, gps):
    # 依原來格式：TIME 3 位小數、PD_TEMP 3，其餘 6
    sys.stdout.write(
