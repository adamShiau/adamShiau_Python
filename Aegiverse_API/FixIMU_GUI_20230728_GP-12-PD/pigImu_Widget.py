...
class pigImuWidget(QWidget):
    ...
    def initUI(self):
        ...
        self.para_block = lineEditBlock(name='parameter configuration file', le_name="parameters_G05")
        self.buffer_lb = displayOneBlock('Buffer Size')
        self.pd_temp_lb = displayOneBlock('PD Temp.', label_size=18)
        ...
        self.plot1_showWz_cb = checkBoxBlock_2('Wz', 'fog', 'mems')
        self.gpstime_lb = displayOneBlock('Data (PC Time)', label_size=18)
        self.plot1_unit_rb = radioButtonBlock_2('Unit', 'dph', 'dps')
        self.plot1 = pgGraph_1_2_FOG(color1="w", color2="r", title="")
        ...
        layout.addWidget(self.read_bt, 0, 4, 1, 2)
        layout.addWidget(self.stop_bt, 0, 6, 1, 2)
        #layout.addWidget(self.save_block, 0, 10, 1, 3)
        layout.addWidget(self.para_block, 0, 8, 1, 5)  # 1, 8, 1, 5
        #layout.addWidget(self.buffer_lb, 1, 4, 1, 1)
        # layout.addWidget(self.pd_temp_lb, 1, 4, 1, 2)
        # layout.addWidget(self.data_rate_lb, 1, 6, 1, 2)
        layout.addWidget(self.pd_temp_lb, 1, 2, 1, 2)
        #layout.addWidget(self.data_rate_lb, 1, 3, 1, 1)
        layout.addWidget(self.logo_lb, 0, 13, 2, 3)
        #layout.addWidget(self.kal_filter_rb, 0, 4, 1, 1)
        layout.addWidget(self.plot1, 2, 0, 10, 16)
        ...
        layout.addWidget(self.gpstime_lb, 1, 4, 1, 9)  # 0, 8, 1, 5
    ...
    def startDisableControlElement(self, en):
        self.para_block.le_filename.setDisabled(en)
