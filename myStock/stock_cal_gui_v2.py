...
class MyStockSimulator(QMainWindow):
    def __init__(self):
        ...
        # 初始標題，後續會動態更換
        self.stock_name = "個股"
        self.setWindowTitle(f"專業投資模擬器 - {self.stock_name}")
    ...
    def load_price(self):
        ...
        if path:
            ...
            self.stock_name = file_name.split('_')[0]
            self.setWindowTitle(f"專業投資模擬器 - {self.stock_name}")
            ...
            if df is not None:
                try:
                    ...
                    QMessageBox.information(self, "成功", f"[{self.stock_name}] 股價載入成功\n範圍: {min_d} ~ {max_d}")
    ...
    def load_div(self):
        ...
        if df is not None:
            ...
            QMessageBox.information(self, "成功", f"[{self.stock_name}] 配息載入完成")
    ...
    def _update_chart(self, s, e):
        ...
        self.ax.plot(sub['Date'], sub['Close'], color='#1f77b4', label=f'{self.stock_name} 歷史趨勢', linewidth=1.5)
        ...
        # 動態替換標題中的名稱
        self.ax.set_title(f"{self.stock_name} 歷史趨勢 ({min_csv_d} ~ {max_csv_d})")
    ...
    def run_simulation_action(self):
        ...
        try:
            ...
            avg_cost = inv / sh if sh > 0 else 0

            # 動態報告標題
            self.lbl_report.setText(
                f"<b>【{self.stock_name} 分析績效報告】</b><br>"
