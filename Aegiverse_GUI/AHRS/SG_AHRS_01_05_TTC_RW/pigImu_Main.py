...
class mainWindow(QMainWindow):
    ...
    def __init__(self, debug_en: bool = False):
        ...
        self.__versionNumber = "SG-AHRS-01-05-TTC-RW"
    ...
    def printUpdateRate(self, t_list):  # 1000007
        try:
            ...
            # logProcess.centrailzedError(num="", fileName=ExternalName_log, content="An error occurred with the array index.", line=tb.tb_lineno)
            logProcess.receive(logName=LOGGER_NAME, num="1000007", fileName=ExternalName_log, error=e)
    ...
    def connectMain(self):  #　1000006
        ...
        if self.__baudRate != "BaudRate(預設)":
            try:
                ...
                self.__portNotErr = False
                logProcess.receive(logName=LOGGER_NAME, num="1000006", fileName=ExternalName_log, error=e)
                # tb = e.__traceback__  # 錯誤軌跡資訊
                # logProcess.centrailzedError(num="", fileName=ExternalName_log, content=f"{type(e).__name__}-An error occurred in the connect.", line=tb.tb_lineno)
            finally:
    ...
    def disconnectMain(self): #　1000005
        try:
            ...
            logProcess.receive(logName=LOGGER_NAME, num="1000005", fileName=ExternalName_log, error=runErr)
            # tb = runErr.__traceback__  # 錯誤軌跡資訊
            # logProcess.centrailzedError(num="1000005", fileName=ExternalName_log, content=f"RuntimeError — {runErr}", line=tb.tb_lineno)
            # logger.error(f"RuntimeError — {runErr}. (An error occurred while in disconnect.)")
    ...
    def occurError(self, errStr: str):
        self.stop()
        self.disconnectMain()
        # logProcess.centrailzedError(err="Error", content="An error occurred causing the GUI to stop and disconnect.", fileName=ExternalName_log)
    ...
    def collectData(self, imudata):  #　1000002
        if not self.press_stop:
            ...
            try:
                ...
                # tb = e.__traceback__    # 錯誤軌跡資訊
                # logProcess.centrailzedError(num="", fileName=ExternalName_log, content=f"{type(e).__name__} - An error occurred while collecting data.", line=tb.tb_lineno)
                logProcess.receive(logName=LOGGER_NAME, num="1000002", fileName=ExternalName_log, error=e)
                self.occurError(errStr="撈取數據功能出現錯誤。")
