...
def errCorrection(isCrcFail, imudata):
    ...
    if not isCrcFail:
        ...
        if err_correction_data == 0:
            err_correction_data = imudata
            for key in err_correction_data:
                if key != "TIME":
                    err_correction_data[key] = 0

        for key in err_correction_data:
            if key != "TIME":
                imudata[key] = err_correction_data[key]
        crcFailCnt += 1
        logger.warning("crc fail: %s", crcFailCnt)
    return imudata
