...
class Connector:
    ...
    def readVersion(self):
        #print("Hi~")
        Ver_text = ""
        VersionByinary = self.readBinaryList(32)
        for i in VersionByinary:
            Ver_text += chr(i)
        #print(Ver_text)
        return Ver_text
    ...
    def readASCIIString(self):
        try:
            raw_data_r = self.__ser.readline()
            raw_data_r = [i for i in raw_data_r]
            #print(raw_data_r)
            if raw_data_r == "":
                self.__portDoNotConnectStatus = 1
        except serial.SerialException:
    ...
    # End of Connector::readASCIIString
...
if __name__ == "__main__":
    ...
    ser = Connector("COM8", 230400)
    ser.connect()
    ser.flushInputBuffer()
    ser.write([171, 186, 101, 0, 0, 0, 0, 2, 85, 86])
    ser.write([171, 186, 1, 0, 0, 0, 5, 2, 85, 86])
    ser.readVersion()
    '''for sparrow test
    ser.write([6, 0, 0, 0, 3])
    time.sleep(1)
