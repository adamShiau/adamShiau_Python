...
class COMRead_Action(QThread):
	...
	def run(self):
		...
		if self.runFlag :
			...
			while self.runFlag:
				if(not TEST_MODE):
					while(not (self.COM.port.inWaiting()>(self.data_frame_update_point*30))) : #rx buffer 不到 arduino傳來的總byte數*data_frame_update_point時不做任何事
				...
				for i in range(0,self.data_frame_update_point): #更新data_frame_update_point筆資料到data and dt array
					...
					if(TEST_MODE):
						...
						# temp_data = self.COM.read12Binary()
					...
					'''
					for i in temp_data:
						SRS200 = np.append(SRS200, i).astype(int)
						# print(hex(i), end='\t')
					print('')
					print('SRS200: ', SRS200)
					while(1):
						for i in range(0, len(SRS200)):
							if( (SRS200[i]==SRS_HEADER)and(SRS200[i+1]==SRS_HEADER) ):
								print('i: ', i)
								break
						break
					for i in range(i+5, i+1, -1):
						data_SRS200 = np.append(data_SRS200, SRS200[i]).astype(int)
					...
					if(DEBUG_COM):
						...
						# print(temp_az)
						pass
					# data = np.append(data[1:], w_SRS200) 
