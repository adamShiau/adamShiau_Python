	...
	def runFog(self):
		...
		if self.runFlag:
			...
			while self.runFlag:
				...
				while(not (self.usb.port.inWaiting()>(60))) : #rx buffer 不到 arduino傳來的總byte數*data_frame_update_point時不做任何事
					# print('buffer:', self.usb.port.inWaiting())
					pass
					
				for j in range(0, 15):
					data_in = self.usb.read4Binary()
					temp = self.usb.read2Binary()
