...
def cic_ii_0_example_design_fir_comp_coeff(L=None, Fs=None, Fc=None, plot=False, is_fxp=True, B=16):
    try:
        ...
        # User Parameters
        if L is None:
            L = 25  # Default filter length
        if L % 2 == 0:
            print(f"FIR filter length must be an odd number. {L + 1} is used instead.")
            L += 1

        if Fs is None:
            Fs = 100e6  # Default sample rate before decimation
        if Fc is None:
            Fc = 5e6  # Default cutoff frequency
        ...
        w, hh = freqz(h, worN=8000)
        # print(h)
        frequencies = w * Fs / (2 * np.pi)
        plt.figure()
        plt.plot(frequencies, 20 * np.log10(abs(hh)), 'b')
...
if __name__ == '__main__':
    cic_ii_0_example_design_fir_comp_coeff(Fc=500 ,plot=False)
