...
# def checkSum(value):
#     # val_Hexadecimal = [ord(str_i) for str_i in value]
#     xor_val = 0
#     # print(val_Hexadecimal)
#     # print([hex(ord(str_i)) for str_i in value])
#     run_num = len(value) - 1
#     for i in range(run_num):
#         if i == 0:
#             xor_val = int(bin(value[i])[2:], 2) ^ int(bin(value[i])[2:], 2)
#         else:
#             xor_val = xor_val ^ int(bin(value[i])[2:], 2)
#     # for i in range(run_num):
#     #     if i == 0:
#     #         xor_val = int(bin(value[i])[2:], 2) ^ int(bin(value[i+1])[2:], 2)
#     #     else:
#     #         xor_val = xor_val ^ int(bin(value[i+1])[2:], 2)
#
#     hex_val = upper(hex(xor_val).replace("0x",""))
#     return hex_val
#     # if hex_val == corret:
#     #     return True
#     # else:
...
def isChecksumFail(value, corret):
    return checkSum(value) != corret
...
def errCorrection(isCrcFail, imudata):
    ...
    if not isCrcFail:
        ...
        crcFailCnt += 1
        logger.warning("crc fail: %d", crcFailCnt)
    return imudata
...
if __name__ == "__main__":
    ...
    data1 = [89, 65, 87, 44, 51, 53, 53, 46, 50, 51]
    result = isChecksumFail(data1, '7F')
    print("checkSum結果: ", result)
