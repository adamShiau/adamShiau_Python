...
class pigImuReader(QThread):
    ...
    def __del__(self):
        logProcess.centrailzedInfo(logName=logger_name, mes="class memsImuReader's destructor called!", fileName=ExternalName_log)
        ...
        while True:
            ...
            for i in range(self.arrayNum):
                ...
                try:
                    ...
                    logProcess.centrailzedDebug(err=Err, content="Key value error. Please verify the error caused by the key value.", fileName=ExternalName_log)
                    self.occurredErr()
                except TypeError as e:
                    # tb = e.__traceback__  # 錯誤軌跡資訊
                    # logProcess.centrailzedError(num="", fileName=ExternalName_log, content="Please check if there is an error in the data type being saved.", line=tb.tb_lineno)
                    logProcess.receive(logName=logger_name, num="1100003", fileName=ExternalName_log, error=e)
                    self.occurredErr()
                finally:
