...
import Sparrow_mini_crc_v2_RT_Action as ACT
...
class mainWindow(QMainWindow):
	...
	def __init__(self, parent = None):
		...
		self.nano33 = np.empty(0)
		self.srs200 = np.empty(0)
	...
	def linkFunction(self):
		...
		if(globals.PRINT_MODE):
			...
			self.act.data_update9.connect(self.plotData)
...
# """ end of comport functin """
	...
	def plotData(self, time, srs200, step, PD_temperature, nano33, speed, ax, ay, az):
		# print('main: \t', end='\t')
		# print(len(time), end='\t')
		# print(len(data), end='\t')
		# print(len(step), end='\t')
		...
		self.top.buffer_lb.lb.setText(str(self.act.bufferSize))
		self.top.temperature_lb.lb.setText(str(PD_temperature[0]))
		self.top.speed_lb.lb.setText(str(speed[0]))
		
		nano33_f = (nano33 + 2.837)*3600
		# data_f = data*ADC_COEFFI 
		time_f = time*TIME_COEFFI
		step_f = (step*self.sf_a_var + self.sf_b_var)*3600

		self.nano33 = np.append(self.nano33, nano33_f)
		self.srs200 = np.append(self.srs200, srs200)
		...
		if (len(self.step) >= 1000):
			self.srs200 = self.srs200[self.act.data_frame_update_point:]
			self.step = self.step[self.act.data_frame_update_point:]
			self.nano33 = self.nano33[self.act.data_frame_update_point:]
		...
		if(self.save_cb_flag == True):
			...
			np.savetxt(self.f, (np.vstack([time_f, srs200, step_f, nano33_f, PD_temperature, 
			ax, ay, az, speed])).T, 
			fmt='%5.5f, %5.5f, %5.5f, %5.5f, %3.1f, %5.5f, %5.5f, %5.5f, %5.5f')
		...
		if(globals.NANO33_WZ_MODE):
			self.top.com_plot1.ax.plot(self.time, self.nano33, color = 'b', linestyle = '-', marker = '', label="nano33")
		if(globals.SRS200_WZ_MODE):
			self.top.com_plot1.ax.plot(self.time, self.srs200, color = 'k', linestyle = '-', marker = '', label="srs200")
