...
class pigImuReader(QThread):
    ...
    def __init__(self, portName: str = "None", boolCaliw=False, boolCalia=False, baudRate: int = 230400,
        ...
        # 2026/01/18 新增
        # --- 新增 Dispatcher 初始化 ---
        self.dispatcher = getData.ProtocolDispatcher()

        # 註冊 IMU 解析器 (Header: 4B, Data: 60B => 總長 64B)
        imu_p = getData.ImuParser(header=[0xFE, 0x81, 0xFF, 0x55], pkt_len=64)
        self.dispatcher.add_parser(imu_p, self.process_imu_packet)

        # 註冊 HINS 解析器
        hins_p = getData.HinsParser()
        self.dispatcher.add_parser(hins_p, self.raw_hins_ack_qt.emit)
