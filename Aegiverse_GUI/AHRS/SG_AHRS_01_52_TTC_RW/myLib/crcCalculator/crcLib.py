...
def crc_32(message, nBytes):
    ...
    try:
        ...
        __logProc = get_WriteToLog()
        __logProc.centrailzedError(err=e, content="Error occurred while verifying CRC.", fileName=ExternalName_log)
...
def errCorrection(isCrcFail, imudata):
    ...
    if not isCrcFail:
        ...
        __logProc = get_WriteToLog()
        __logProc.centrailzedWarning(content=f'crc fail: {str(crcFailCnt)}', fileName=ExternalName_log)
    return imudata
