...
if os.getenv('verNum') == str(True) :
    ExternalName_log = "SerCOM_logger"
...
class usbConnect():
    def __init__(self, group_name='Connect COM port'):
        ...
        # baudrate
        self.B_Rate = QComboBox()
        self.B_Rate.addItem("BaudRate(預設)")
        self.B_Rate.addItem("115200")
        self.B_Rate.addItem("230400")
        self.B_Rate.addItem("460800")
        self.B_Rate.setCurrentText("BaudRate(預設)")
    ...
    def layout(self):
        ...
        layout.addWidget(self.B_Rate, 0, 2, 1, 1)
        layout.addWidget(self.bt_connect, 0, 3, 1, 1)
        layout.addWidget(self.bt_disconnect, 0, 4, 1, 1)
        layout.addWidget(self.lb_comDisp, 1, 0, 1, 2)
        layout.addWidget(self.lb_status, 1, 3, 1, 2)
    ...
    def addPortItems(self, num, ports):
        ...
        self.cb.clear()
        logProcess.checkInfoDebug(logName=logger_name, content=f'port_num: {num}', fileName=ExternalName_log)
        ...
        if num > 0:
            for i in range(num):
                ...
                logProcess.checkInfoDebug(logName=logger_name, content=f'port_name: {ports[i].device}', fileName=ExternalName_log)
                logProcess.checkInfoDebug(logName=logger_name, content=f'ports.description: {ports[i].description}', fileName=ExternalName_log)
                logProcess.checkInfoDebug(logName=logger_name, content=f'ports.hwid: {ports[i].hwid}', fileName=ExternalName_log)
                logProcess.checkInfoDebug(logName=logger_name, content=f'ports.serial_number: {ports[i].serial_number}', fileName=ExternalName_log)
                logProcess.checkInfoDebug(logName=logger_name, content=f'ports.product: {ports[i].product}', fileName=ExternalName_log)
                logProcess.checkInfoDebug(logName=logger_name, content=f'ports.manufacturer: {ports[i].manufacturer}', fileName=ExternalName_log)
                logProcess.checkInfoDebug(logName=logger_name, content=f'ports.location: {ports[i].location}\n', fileName=ExternalName_log)
    ...
    def selectBRate(self):
        rate = self.B_Rate.currentText()
        return rate
...
class usbConnect_auto():
    ...
    def autoComport(self, num, ports):
        self.__portList = ports
        # self.cb.clear()
        logProcess.checkInfoDebug(content=f'port_num: {num}', fileName=ExternalName_log)
        # port = dict()
        if num > 0:
            for i in range(num):
        ...
        logProcess.checkInfoDebug(logName=logger_name, content=f'autoComport: {self.__port}\n', fileName=ExternalName_log)
...
class spinBlock(QGroupBox):
    def __init__(self, title, minValue, maxValue, step=1, double=False, Decimals=2):
        ...
        self.spin.wheelEvent = self.disableWheelEvent
        self.setTitle(title)
    ...
    def disableWheelEvent(self, event):
        event.ignore()
