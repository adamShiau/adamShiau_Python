...
def saveTempFile(isopen: bool = False, data: str = None, file: object = None):
    if isopen:
        file.write(data + "\n")
...
class data_manager:
    ...
    def close(self):
        if self.__isopen:
            # date_now = datetime.now().strftime("%Y/%m/%d %H:%M:%S")
            date_now = (datetime.now().strftime("%Y/%m/%d %H:%M:%S:%f")[:-3]) # 20230717添加
            self.__fd.writelines('#' + date_now + '\n')
            # print('close !!')
    ...
    def saveTemp(self, tempdata):  # 20230712
        saveTempFile(isopen=self.__isopen, data=tempdata, file=self.__fd)

class json_manager:
    def __init__(self, name="dataFill.txt"):
        self.__name__ = name

    def writeJson(self, data):
        with open(self.__name__, 'w') as f:
            json.dump(data, f)
...
def dictOperation(dictA: dict, dictB: dict, mode: str, dictStruct: dict):
    ...
    #         print_debug("dictA[k]= " + str(dictA[k]), PRINT_DEBUG)
    #         print_debug("dictB[k]= " + str(dictB[k]), PRINT_DEBUG)
    #         if k == "MON" or k == "DAY" or k == "HOUR" or k == "MIN" or k == "SEC" or k == "mSEC" or k == "mSEC" or k == "time_accumulate":
