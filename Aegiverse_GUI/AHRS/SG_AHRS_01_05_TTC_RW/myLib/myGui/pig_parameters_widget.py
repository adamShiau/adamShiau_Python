...
class pig_parameters_widget(QGroupBox):
    ...
    def initUI(self):
        ...
        mainLayout.addWidget(self.Firmware_Version_lb, 10, 0, 1, 4)
        mainLayout.addWidget(self.GUI_Version_lb, 11, 0, 1, 4)
    ...
    def checkKeyExist(self, para, key):
        ...
        if not isinstance(val, pd.Series):
            if val == None:
                ...
                return None
            if val == "nan" or val == '':
                logProcess.centrailzedInfo(logName=logger_name, mes=f"{key}--參數值為空的狀況，故回傳數值0代替空值。", fileName=ExternalName_log)
        ...
        if filename != "":
            ...
            if isinstance(tempVal, pd.Series) and isinstance(slopeVal, pd.Series) and isinstance(offsetVal, pd.Series):
                try:
                    ...
                    logProcess.receive(logName=logger_name, num="", fileName=ExternalName_log, error=e)
                    # tb = e.__traceback__  # 錯誤軌跡資訊
                    # logProcess.centrailzedError(num="", fileName=ExternalName_log, content=f"{type(e).__name__}--載入CSV數據的功能出現錯誤。", line=tb.tb_lineno)
            else:
                self.keyIsNetExist = True
    ...
    def updateControl(self, controlItem, para, keyVal):
        if not controlItem.property("called_once"):
            try:
                ...
                tb = e.__traceback__  # 錯誤軌跡資訊
                logProcess.centrailzedError(logName=logger_name, num="", fileName=ExternalName_log, content=f"請確認計算使用的變數數值，是否型態是相同的。", line=tb.tb_lineno)
        else:
            logProcess.checkInfoDebug(content="重複使用控制項了，請確認程式碼是否有使用到重複的控制項。", fileName=ExternalName_log)
    ...
    def send_FREQ_CMD(self):
        ...
        logProcess.centrailzedInfo(mes=f'set freq: {value}', fileName=ExternalName_log)
    ...
    def send_MOD_H_CMD(self):
        ...
        logProcess.centrailzedInfo(mes=f'set mod_H: {value}', fileName=ExternalName_log)
    ...
    def send_MOD_L_CMD(self):
        ...
        logProcess.centrailzedInfo(mes=f'set mod_L: {value}', fileName=ExternalName_log)
    ...
    def send_ERR_OFFSET_CMD(self):
        ...
        logProcess.centrailzedInfo(mes=f'set err offset: {value}', fileName=ExternalName_log)
    ...
    def send_POLARITY_CMD(self):
        ...
        logProcess.centrailzedInfo(mes=f'set polarity: {value}', fileName=ExternalName_log)
    ...
    def send_WAIT_CNT_CMD(self):
        ...
        logProcess.centrailzedInfo(mes=f'set wait cnt: {value}', fileName=ExternalName_log)
    ...
    def send_ERR_TH_CMD(self):
        ...
        logProcess.centrailzedInfo(mes=f'set err_th: {value}', fileName=ExternalName_log)
    ...
    def send_AVG_CMD(self):
        ...
        logProcess.centrailzedInfo(mes=f'set err_avg: {value}', fileName=ExternalName_log)
    ...
    def send_GAIN1_CMD(self):
        ...
        logProcess.centrailzedInfo(mes=f'set gain1: {value}', fileName=ExternalName_log)
    ...
    def send_GAIN2_CMD(self):
        ...
        logProcess.centrailzedInfo(mes=f'set gain2: {value}', fileName=ExternalName_log)
    ...
    def send_FB_ON_CMD(self):
        ...
        logProcess.centrailzedInfo(mes=f'set FB on: {value}', fileName=ExternalName_log)
    ...
    def send_DAC_GAIN_CMD(self):
        ...
        logProcess.centrailzedInfo(mes=f'set DAC gain: {value}', fileName=ExternalName_log)
    ...
    def send_CUTOFF_CMD(self):
        ...
        logProcess.centrailzedInfo(mes=f'set CUTOFF : {value}', fileName=ExternalName_log)
        # value2 = struct.unpack('<I', struct.pack('<f', value))
    ...
    def send_SF0_CMD(self):
        ...
        logProcess.centrailzedInfo(mes=f'set SFA : {Re_sf0Val}', fileName=ExternalName_log)
    ...
    def send_SF1_CMD(self):
        ...
        logProcess.centrailzedInfo(mes=f'set SFB : {Re_sf1Val}', fileName=ExternalName_log)
    ...
    def send_BIAS_T1_CMD(self):
        ...
        logProcess.centrailzedInfo(mes=f'set BIAS T1 : {float(self.T1.le.text())}', fileName=ExternalName_log)
    ...
    def send_BIAS_T2_CMD(self):
        ...
        logProcess.centrailzedInfo(mes=f'set BIAS T2 : {float(self.T2.le.text())}')
    ...
    def send_SFB_SLOPE_1_CMD(self):
        ...
        logProcess.centrailzedInfo(mes=f'set BIAS SF SLOPE1 : {float(self.slope1.le.text())}')
    ...
    def send_SFB_SLOPE_2_CMD(self):
        ...
        logProcess.centrailzedInfo(mes=f'set BIAS SF SLOPE2 : {float(self.slope2.le.text())}')
    ...
    def send_SFB_SLOPE_3_CMD(self):
        ...
        logProcess.centrailzedInfo(mes=f'set BIAS SF SLOPE3 : {float(self.slope3.le.text())}')
    ...
    def send_SFB_OFFSET_1_CMD(self):
        ...
        logProcess.centrailzedInfo(mes=f'set BIAS SF OFFSET1 : {float(self.offset1.le.text())}')
    ...
    def send_SFB_OFFSET_2_CMD(self):
        ...
        logProcess.centrailzedInfo(mes=f'set BIAS SF OFFSET2 : {float(self.offset2.le.text())}')
    ...
    def send_SFB_OFFSET_3_CMD(self):
        ...
        logProcess.centrailzedInfo(mes=f'set BIAS SF OFFSET3 : {float(self.offset3.le.text())}', fileName=ExternalName_log)
    ...
    def send_CONST_STEP_CMD(self):
        ...
        logProcess.centrailzedInfo(mes=f'set constant step: {value}', fileName=ExternalName_log)
    ...
    def update_KF_Q(self):
        ...
        logProcess.centrailzedInfo(mes=f'set KF_Q: {value}', fileName=ExternalName_log)
    ...
    def update_KF_R(self):
        ...
        logProcess.centrailzedInfo(mes=f'set KF_R: {value}', fileName=ExternalName_log)
    ...
    def update_FPGA_Q(self):
        ...
        logProcess.centrailzedInfo(mes=f'set HD_Q: {value}', fileName=ExternalName_log)
    ...
    def update_FPGA_R(self):
        ...
        logProcess.centrailzedInfo(mes=f'set HD_R: {value}', fileName=ExternalName_log)
    ...
    def send_DATA_RATE_CMD(self):
        ...
        logProcess.centrailzedInfo(mes=f'set dataRate: {value}', fileName=ExternalName_log)
