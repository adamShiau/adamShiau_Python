...
''' for X axis SF temp comp.'''
# SF_p0 = 0.742982
# SF_p1 = 0.00001983288
# SF_p2 = 0.000000156571
# SF_p3 = -0.000000001536975

''' for X BIAS temp comp.'''
# BS_p0 = 1433.1
# BS_p1 = 7.581278
# BS_p2 = -0.06303523
# BS_p3 = 0.0002968801
Tx2 = Tx-30
# SF_comp_X = (SF_p0 + SF_p1*Tx2 + SF_p2*Tx2**2 + SF_p3*Tx2**3)
# BS_comp_X = (BS_p0 + BS_p1*Tx2 + BS_p2*Tx2**2 + BS_p3*Tx2**3)/1000  # mg

''' for Z axis SF temp comp.'''
SF_p0 = 0.72498
SF_p1 = 0.00001024376
SF_p2 = 0.0000002348138
SF_p3 = -0.000000001456818

''' for Z BIAS temp comp.'''
BS_p0 = 2624.23  # ug
BS_p1 = 23.27427
BS_p2 = -0.06018567
BS_p3 = -0.001522216
...
SF_comp_Z = (SF_p0 + SF_p1*Tz2 + SF_p2*Tz2**2 + SF_p3*Tz2**3)
BS_comp_Z = (BS_p0 + BS_p1*Tz2 + BS_p2*Tz2**2 + BS_p3*Tz2**3)  # ug
# print(SF_comp_Z)
# print(BS_comp_Z)
az_noComp = (az*SF_CODE)*1000/R_MEAS/SF_p0 - BS_p0/1000000
# print(az*SF_CODE, SF_p0, BS_p0)
az_Comp = (az*SF_CODE)*1000/R_MEAS/SF_comp_Z - BS_comp_Z/1000000
# print(az*SF_CODE, SF_comp_Z, BS_comp_Z)

# plt.figure(1)
# plt.plot(time, az_noComp, label='az_noComp')
# plt.legend()
...
# 在第一个 y 轴上绘制 az 数据
ax1.plot(time, az_noComp, 'g-', label='az_noComp')  # 绿色线条表示 az 数据
ax1.set_xlabel('time(hrs)')          # 设置 x 轴标签
ax1.set_ylabel('g', color='g')    # 设置左侧 y 轴标签和颜色
...
fig, ax1 = plt.subplots()
# 在第一个 y 轴上绘制 az 数据
ax1.plot(time, az_Comp, 'g-', label='az_Comp')  # 绿色线条表示 az 数据
ax1.set_xlabel('time(hrs)')          # 设置 x 轴标签
ax1.set_ylabel('g', color='g')    # 设置左侧 y 轴标签和颜色
ax1.tick_params(axis='y', labelcolor='g')
# 设置 Tz 轴的 y 轴范围
...
# plt.figure(2)
# plt.plot(time, az_Comp, label='az_Comp')
# plt.legend()



# SFA = SFA1*100/SF_comp_Z
# SFB = SFB1*100/SF_comp_Z - BS_comp_Z
...
with open('XLM550_0924_DiracADC_1kohm_100Hz_gcomp.txt', 'w') as f:
    ...
    for i in range(len(time)):
        f.write(
            f"{time[i]:.3f},{wx[i]:.5f},{wy[i]:.5f},{wz[i]:.5f},{ax[i]:.5f},{ay[i]:.5f},{az_Comp[i]:.5f},{Tx[i]:.2f},{Ty[i]:.2f},{Tz[i]:.2f}\n")
