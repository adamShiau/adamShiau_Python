...
def readAFI(dataPacket, POS_WX, POS_WY, POS_WZ, POS_A, POS_TIME, POS_TEMPX, POS_TEMPY, POS_TEMPZ,
    ...
    Tx = IEEE_754_INT2F_MSB(temp_Tx)
    Ty = IEEE_754_INT2F_MSB(temp_Ty)
    Tz = IEEE_754_INT2F_MSB(temp_Tz)
    mcu_time = convert2Unsign_4B_R(temp_time) / 1000.0
    if PRINT:
        print('\n%f, ' % mcu_time, end=', ')
        print('%f, ' % wx, end=', ')
...
def readAFI_ATT_PDf(dataPacket, POS_WX, POS_WY, POS_WZ, POS_AX, POS_AY, POS_AZ, POS_TIME, POS_TEMPX, POS_TEMPY, POS_TEMPZ,
            POS_PITCH, POS_ROW, POS_YAW, dataLen, PRINT=0):
    temp_wx = dataPacket[POS_WX:POS_WX + dataLen]
    temp_wy = dataPacket[POS_WY:POS_WY + dataLen]
    temp_wz = dataPacket[POS_WZ:POS_WZ + dataLen]
    ...
    temp_row = dataPacket[POS_ROW:POS_ROW + dataLen]
    ...
    return mcu_time, wx, wy, wz, ax, ay, az, Tx, Ty, Tz, pitch, row, yaw
...
class data_manager:
    ...
    def open(self, status):
        ...
        if self.__isopen:
            ...
            self.__openTime__ = datetime.strptime(date_now, '%Y/%m/%d %H:%M:%S')
    ...
    # 用於將dump的數據寫入txt檔案的function
    def write_dump(self, comment):
        if self.__isopen:
            self.__fd.writelines('#' + str(comment) + '\n')
...
def IEEE_754_INT2F_R(datain): # LSB
    if len(datain) == 4:
        # shift_data = (datain[0] << 24 | datain[1] << 16 | datain[2] << 8 | datain[3])
        shift_data = (datain[3] << 24 | datain[2] << 16 | datain[1] << 8 | datain[0])
        f = struct.unpack('<f', struct.pack('<I', shift_data))
...
def IEEE_754_INT2F_MSB(datain): # MSB封包格式
    if len(datain) == 4:
        shift_data = (datain[0] << 24 | datain[1] << 16 | datain[2] << 8 | datain[3])
        f = struct.unpack('<f', struct.pack('<I', shift_data))
        return f[0]
...
def convert2Sign_4B(datain): # MSB
    if len(datain) == 4:
        shift_data = (datain[0] << 24 | datain[1] << 16 | datain[2] << 8 | datain[3])
        if (datain[0] >> 7) == 1:
