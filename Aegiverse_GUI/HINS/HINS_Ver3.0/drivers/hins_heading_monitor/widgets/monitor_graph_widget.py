...
class MonitorGraphWidget(QWidget):
    ...
    def setup_ui(self):
        ...
        # 彈簧 1：將 Checkbox 推向最左邊
        ctrl_layout.addStretch(1)

        # [更新] 正式數值顯示 (Dark Colors)
        self.frame_da, self.lbl_cur_da = self.create_formal_label("DA Heading", "#008080")
        self.frame_imu, self.lbl_cur_imu = self.create_formal_label("IMU Heading", "#8B4513")
        self.frame_off, self.lbl_cur_off = self.create_formal_label("G-Offset", "#800080")

        ctrl_layout.addWidget(self.frame_da)
        ctrl_layout.addSpacing(40)  # [更新] 顯著拉開數值間距
    ...
    def update_data(self, data):
        """ [完整保留] 數據更新邏輯，並加入 Case Flag 與正式 Label 更新 """
        try:
            # 1. 狀態與名稱更新
            ...
            # [優化] 只處理 Bit 2-15 的警告邏輯
            # 使用 0xFFFC (1111 1111 1111 1100) 過濾掉 Bit 0 與 Bit 1
            has_warning = (s82 & 0xFFFC) != 0
            ...
            if s82 & (1 << 6):  w_names.append("Height")
            if s82 & (1 << 7):  w_names.append("Gyr_Bias")
            if s82 & (1 << 8):  w_names.append("Acc_Bias")
            if s82 & (1 << 9):  w_names.append("GNSS_Err")
            if s82 & (1 << 10): w_names.append("NoTimeSync")
            if s82 & (1 << 11): w_names.append("Sensor_Err")

            # 顯示處理：如果有任何 Bit 2-15 被觸發，顯示警告名稱；否則顯示 Stable
            # 這裡依然保留 hex(s82) 方便您對照原始數據
            status_text = f"{hex(s82)} ({', '.join(w_names)})" if has_warning else f"{hex(s82)} (Stable)"
            self.le_widgets["warn"].setText(status_text)

            cond = s82 & 0x03;
            self.le_widgets["cond"].setText(f"{cond} ({self.COND_NAMES.get(cond, 'UNK')})")

            # 同時顯示 HEX 與 Stable/Warning
            self.le_widgets["warn"].setText(f"{hex(s82)} ({', '.join(w_names) if w_names else 'Stable'})")
            ...
            self.lbl_cur_da.setText(f"{h_da:.2f}")
            self.lbl_cur_imu.setText(f"{h_imu:.2f}")
            self.lbl_cur_off.setText(f"{h_off:.2f}")
