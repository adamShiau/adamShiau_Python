...
def plot_with_separators(data, x_col, y_cols, labels, colors, title, ylim=None):
    """
    繪圖函數，支持添加虛線分隔和設置 Y 軸範圍。

    :param data: 數據框
    :param x_col: X 軸欄位
    :param y_cols: Y 軸欄位列表
    :param labels: Y 軸標籤列表
    :param colors: 顏色列表
    :param title: 圖表標題
    :param ylim: Y 軸範圍 (tuple, optional) (y_min, y_max)
    """
    ...
    if ylim:
        plt.ylim(ylim)
# def plot_with_separators(data, x_col, y_cols, labels, colors, title):
#     plt.figure(figsize=(12, 6))
#     files = data["File_Name"].unique()
#     for file_name in files:
#         file_data = data[data["File_Name"] == file_name]
...
# 第二張圖：FG_Yaw 與 Heading_Reversed
# plot_with_separators(
#     combined_data_cleaned,
#     "Relative_Timestamp",
#     ["FG_Yaw", "Heading_Reversed"],
#     ["FG_Yaw", "Heading Reversed"],
#     ["blue", "red"],
#     "FG_Yaw and Heading_Reversed vs Relative Timestamp"
# )

# 第三張圖：FG_Yaw 與 Heading_Reversed_with_Offset
plot_with_separators(
    combined_data_cleaned,
    "Relative_Timestamp",
