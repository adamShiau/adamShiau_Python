...
def crc_32(message, nBytes):
    ...
    try:
        ...
        # logProcess.centrailzedError(num="", fileName=ExternalName_log, content="IndexError-Please ensure that the array contains a value at the specified index. "
        #                                             "(The issue appears to originate from the CRC file.)", line=tb.tb_lineno)
    except TypeError as e:
        logProcess.receive(logName=logger_name, num="", fileName=ExternalName_log, error=e)
        # tb = e.__traceback__  # 錯誤軌跡資訊
        # logProcess.centrailzedError(num="", fileName=ExternalName_log, content="Ensure that all operands are of the same type before performing the calculation."
        #                                             "(The issue appears to originate from the CRC file.)", line=tb.tb_lineno)
...
def errCorrection(isCrcFail, imudata):
    ...
    if not isCrcFail:
        ...
        crcFailCnt += 1
        logProcess.centrailzedWarning(logName=logger_name, content=f'crc fail: {str(crcFailCnt)}', fileName=ExternalName_log)
...
def errCorrection_vbox(isCrcFail, vbox_data):
    ...
    if not isCrcFail:
        ...
        logProcess.centrailzedWarning(logName=logger_name, content=f"vbox crc fail: {str(crcFailCnt_vbox)}", fileName=ExternalName_log)
    return vbox_data
