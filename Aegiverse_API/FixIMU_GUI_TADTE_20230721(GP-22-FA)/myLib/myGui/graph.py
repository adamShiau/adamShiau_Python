...
class pgGraph_1_W(QMainWindow):
    def __init__(self, color="w", title="add title"):
        super(pgGraph_1_W, self).__init__()
        self.ax = None
        win = pg.GraphicsWindow()
        win.setBackground('k')
        pen = pg.mkPen(color=color, width=1)
        self.p = win.addPlot(title=title)
        self.ax = self.p.plot(pen=pen)
        # self.p.getAxis('left').setLabel('Unit：dph')
        # self.p.getAxis('bottom').setLabel('Unit：time')
        self.setCentralWidget(win)

class pgGraph_1_ADXL(QMainWindow):
    def __init__(self, color="w", title="add title"):
        super(pgGraph_1_ADXL, self).__init__()
        self.ax = None
        win = pg.GraphicsWindow()
        win.setBackground('k')
        pen = pg.mkPen(color=color, width=1)
        self.p = win.addPlot(title=title)
        self.ax = self.p.plot(pen=pen)
        # self.p.getAxis('left').setLabel('Unit：g')
        # self.p.getAxis('bottom').setLabel('Unit：time')
        self.setCentralWidget(win)
...
class pgGraph_1_3(QMainWindow):
    def __init__(self, color1="w", color2="w", color3="w", width1=1, width2=1, width3=1, linename1="line 1", linename2="line 2", linename3="line 3" , title="add title"):
        ...
        self.p.getAxis('bottom').setLabel('Time(sec)')
        self.setCentralWidget(win)
...
# 航太展使用
class pgGraph_1_2(QMainWindow):
    def __init__(self, color1="w", color2="w", width1=1, width2=1):
        super(pgGraph_1_2, self).__init__()
        ...
        # label = pg.LabelItem()
        # label.setText('<span style="color: red;">red：MEMS_WZ</span><br><span style="color: white;">white：FOG_WZ</span>')
        # win.addItem(label)
        self.p.getAxis('left').setLabel('Unit：dph')
        self.p.getAxis('bottom').setLabel('Unit：time')
        # print(type(self.ax1))
...
class pgGraph_vbox_2(QMainWindow):
    def __init__(self, color1="w", color2="w", width1=1, width2=1, title="add title"):
        super(pgGraph_vbox_2, self).__init__()
...
class pgGraph_circle_1_2(QMainWindow):
    def __init__(self, color1="w", color2="w", width1=1, width2=1, title="add title"):
        super(pgGraph_circle_1_2, self).__init__()
        ...
        self.ax2 = self.p.plot(pen=pen2)
        label = pg.LabelItem()
        label.setText('<span style="color: white;">white：MEMS</span><br><span style="color: red;">red：FOG</span>')
        win.addItem(label)
        self.p.getAxis('left').setLabel('Unit：meter')
        self.p.getAxis('bottom').setLabel('Unit：meter')
        # print(type(self.ax1))
        self.setCentralWidget(win)
