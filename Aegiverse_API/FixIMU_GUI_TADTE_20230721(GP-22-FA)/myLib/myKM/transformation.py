...
class Quaternion:
    ...
    def multiply(self, arg):
        ...
        if isinstance(arg, Quaternion):
            ...
            if arg.shape == (3,):
                ...
                vector = self.FourByFour(vector, self.inv().q)
                return vector[:-1]
            else:
                new_q = self.FourByFour(self.q, arg)
                return new_q
    ...
    def getDeltaQuaternion(self, omg, dt):
        """used for Quaternion multiplying"""
        q_0 = self.q.copy()
        self.rotate(*omg, dt)
        dif_q = Quaternion(q_0).inv().multiply(self)
        return dif_q

    def getDeltaQuaternion2(self, omg, dt):
        """user to add Quaternion """
        # the angle of rotation respect to b-frame
        [wx, wy, wz] = omg
        new_omg = np.array([[0, wz, -wy, wx],
                            [-wz, 0, wx, wy],
