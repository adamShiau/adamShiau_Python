...
def readMP_1Z(dataPacket, POS_WX, POS_WY, POS_WZ, POS_AX, POS_AY, POS_AZ, POS_TIME, POS_PDTEMP, dataLen, PRINT=0):
    ...
    mcu_time = convert2Unsign_4B_R(temp_time) / 1000.0
    # print(hex(temp_wz[0]), end=', ')
    # print(hex(temp_wz[1]), end=', ')
    # print(hex(temp_wz[2]), end=', ')
    # print(hex(temp_wz[3]))
    if PRINT:
        print('\n%f, ' % mcu_time, end=', ')
        print('%f, ' % wx, end=', ')
...
def readVN_300(dataPacket, HEADER_BYTE=4, PRINT=0):
    temp_time = dataPacket[HEADER_BYTE:HEADER_BYTE + 8]
    temp_yaw = dataPacket[HEADER_BYTE + 8:HEADER_BYTE + 12]
    temp_pitch = dataPacket[HEADER_BYTE + 12:HEADER_BYTE + 16]
    temp_row = dataPacket[HEADER_BYTE + 16:HEADER_BYTE + 20]

    temp_wx = dataPacket[HEADER_BYTE + 20:HEADER_BYTE + 24]
    temp_wy = dataPacket[HEADER_BYTE + 24:HEADER_BYTE + 28]
    temp_wz = dataPacket[HEADER_BYTE + 28:HEADER_BYTE + 32]
    temp_ax = dataPacket[HEADER_BYTE + 32:HEADER_BYTE + 36]
    temp_ay = dataPacket[HEADER_BYTE + 36:HEADER_BYTE + 40]
    temp_az = dataPacket[HEADER_BYTE + 40:HEADER_BYTE + 44]
    temp_pdtemp = dataPacket[HEADER_BYTE + 44:HEADER_BYTE + 48]


    # print("temp_time (HEX):", " ".join(f"{b:02X}" for b in temp_time))
    ...
    mcu_time = convert2Unsign_8B_R(temp_time) / 1000000000.0
...
def readVBOX(dataPacket, POS_GPS_SATS, POS_GLONASS_SATS, POS_BeiDou_SATS, POS_Z_accel, POS_Time, POS_Heading,
    ...
    if EN:
        ...
        logProcess.centrailzedInfo('readVBOX EN = 0')
    # End of if-condition
...
def file_manager(isopen=False, name="notitle", mode="w", fnum=0):
    ...
    if isopen:
        ...
        try:
            ...
            logger.error("1500001, NameError")
            pass
        except AttributeError as e:
            logger.error("1500001, AttributeError - the file attempt to close does not exist!")
...
def loadCSVFile(filename):  # 1500002
    try:
        filePath = filename
        dataContent = pd.read_csv(filePath)
    except pd.errors.ParserError as e:
        logger.error(f'1500002, The format of the loaded file is incorrect. Please verify the content format.')
        return None
    except pd.errors.EmptyDataError as e:
        logger.error(f'1500002, The content of the file is empty or contains only the header row.')
        return None
...
def convert2Unsign_8B_R(datain):
    """將 8 Byte 小端序 (Little Endian) 轉成 64-bit 無號整數"""
    if len(datain) != 8:
        raise ValueError(f"Input length must be 8 bytes, got {len(datain)}")
...
def IEEE_754_INT2F(datain):
    if len(datain) == 4:
        ...
        try:
            ...
            logger.error("An error occurred while converting data using struct.-1")
            return -1
        # print('%.3f' % f[0])
        return f[0]
    else:
...
def IEEE_754_INT2F_R(datain):
    if len(datain) == 4:
        ...
        try:
            ...
            logger.error(f'An error occurred while converting data using struct.-2')
            return -1
        # print('%.3f' % f[0])
        return f[0]
    else:
