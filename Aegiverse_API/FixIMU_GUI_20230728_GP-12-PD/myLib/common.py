...
def readPIG_onlyAngle(dataPacket, EN=1, POS_TIME=5, PRINT=0):
    if EN:
        try:
            temp_angle = dataPacket[POS_TIME:POS_TIME+6]
            FPGA_angle = processingAngle(temp_angle)
        except IndexError:
            return 10000
    else:
        FPGA_angle = 0
...
# def changeToASCII(val):
#     str_val = ""
#     for i in range(6):
#         #byteData = bytes.fromhex(val[i])
#         byteData = chr(val[i])
#         #str_val += byteData.decode('ascii')
#
#     IntVal = float(byteData)
#     print(str_val)
#     return IntVal
def processingAngle(val):
    str_val = ""
    for i in range(6):
        #print(val[i])
        if str(chr(val[i])) != " " and str(chr(val[i])) != "«" and str(chr(val[i])) != "º" and str(chr(val[i])) != ",":
            str_val += str(chr(val[i]))
            #print(str_val)

    IntVal = float(str_val)
    #print(str_val)
    return IntVal

def changeToASCII(dataPacket):
    ASCII_str = ""
    for i in range(len(dataPacket[:14])):
        ASCII_str += str(chr(dataPacket[i]))
    #print(ASCII_str)
    return ASCII_str

def changToHex(dataPacket):
    Hex_str = ""
    for i in range(len(dataPacket)):
        HexVal = str(hex(dataPacket[i])).replace("0x", "").upper()
        if len(HexVal) != 2:
            HexVal = "0"+HexVal
        Hex_str += HexVal
        if i != (len(dataPacket) - 1):
            Hex_str += ","
