...
if os.getenv('verNumIsOut') == str(True) :
    ExternalName_log = "anaAll_logger"
...
class analysis_allan_widget(QWidget):
    ...
    def plot(self, tau, dev):
        ...
        # self.allan_plot.ax.loglog(tau, dev * 1, 'k-*')  # convert unit to dph
        self.plot_control()
        self.allan_plot.fig.canvas.draw()
...
class allan_dev(QThread):
    allan_qt = Signal(object, object)
    finish_qt = Signal(object, object)
    # tauarray_qt = pyqtSignal(object)
    progress_qt = Signal(int, int)
    is_allan_done_qt = Signal(bool)
