...
class Connector:
    ...
    def __del__(self):
        # self.disconnect()
        logProcess.centrailzedInfo(logName=logger_name, mes="class connector's destructor called!", fileName=ExternalName_log)
    ...
    def connectConn(self): # 1700001
        ...
        try:
            ...
            # logProcess.centrailzedError(num="1700001", fileName=ExternalName_log, content="The serial open operation failed while establishing the connection.", line=tb.tb_lineno)
            logProcess.receive(logName=logger_name, num="1700001", fileName=ExternalName_log, error=e)
            return False
        except IOError as e:
            tb = e.__traceback__  # 錯誤軌跡資訊
            # 發生此錯誤不需要把GUI整個關掉
            logProcess.centrailzedError(logName=logger_name, num="1700001", fileName=ExternalName_log, content="IOError - the device: " + self.__ser.port + " can not be found or can not be configured!", line=tb.tb_lineno)
            # sys.exit(0)
            return False
        # End of try-catch
        logProcess.centrailzedInfo(logName=logger_name, mes=self.__ser.port + " is connected", fileName=ExternalName_log)
    ...
    def disconnectConn(self):
        try:
            if self.__is_open:
                self.__ser.close()
            self.__is_open = self.__ser.is_open
            logProcess.centrailzedInfo(logName=logger_name, mes=self.__ser.port + " is disconnected", fileName=ExternalName_log)
        except serial.SerialException as serialErr:
            # tb = serialErr.__traceback__  # 錯誤軌跡資訊
            # logProcess.centrailzedError(num="1700006", fileName=ExternalName_log, content="The serial close operation failed when disconnecting the connection.", line=tb.tb_lineno)
            logProcess.receive(logName=logger_name, num="1700006", fileName=ExternalName_log, error=serialErr)
        return self.__is_open
    ...
    def write(self, data_w):  # 1700002
        try:
            ...
            tb = serErr.__traceback__  # 錯誤軌跡資訊
            logProcess.centrailzedError(logName=logger_name, num="1700002", fileName=ExternalName_log, content=f"{type(serErr).__name__} - write timeOut", line=tb.tb_lineno)
        except serial.PortNotOpenError as serErr:
            tb = serErr.__traceback__  # 錯誤軌跡資訊
            logProcess.centrailzedError(logName=logger_name, num="1700002", fileName=ExternalName_log, content=f"{type(serErr).__name__} - Port not open, please check!", line=tb.tb_lineno)
        except serial.SerialException as serErr:
            tb = serErr.__traceback__  # 錯誤軌跡資訊
            logProcess.centrailzedError(logName=logger_name, num="1700002", fileName=ExternalName_log, content=f"{type(serErr).__name__} - The device can not be found or can not be configured.", line=tb.tb_lineno)
        except AttributeError as e:
            # tb = e.__traceback__  # 錯誤軌跡資訊
            # logProcess.centrailzedError(num="1700002", fileName=ExternalName_log, content=f"The error is '{e}'", line=tb.tb_lineno)
            logProcess.receive(logName=logger_name, num="1700002", fileName=ExternalName_log, error=e)
    ...
    def readBinaryList(self, mum):
        try:
            ...
            # tb = serialTimeout.__traceback__  # 錯誤軌跡資訊
            # logProcess.centrailzedError(num="1700003", fileName=ExternalName_log, content="ERROR", line=tb.tb_lineno)
            logProcess.receive(logName=logger_name, num="1700003", fileName=ExternalName_log, error=serialTimeout)
            return False
        # data = [hex(i) for i in data]
        except serial.serialutil.SerialException as serException:
            logProcess.receive(logName=logger_name, num="1700003", fileName=ExternalName_log, error=serException)
            return False
        except AttributeError as attributeErr:
            # tb = attributeErr.__traceback__  # 錯誤軌跡資訊
            # logProcess.centrailzedError(num="1700003", content="Please make sure the device is connected.", fileName=ExternalName_log, line=tb.tb_lineno)
            logProcess.receive(logName=logger_name, num="1700003", fileName=ExternalName_log, error=attributeErr)
            return False
        # except Exception as e:
        ...
        #     return False
            ...
            if not data_r:
                ...
                # 此error為ValueError
                logProcess.centrailzedWarning(logName=logger_name, content='serial read timeout: check if the input power is ON, close the GUI and re-excute.',fileName=ExternalName_log)
                return False
            return data_r
    ...
    def readInputBuffer(self):
        ...
        try:
            ...
            # tb = e.__traceback__  # 錯誤軌跡資訊
            # # 錯誤:序列埠的連接、斷線或是被占用時，當執行此程式會出現的錯誤。
            # logProcess.centrailzedError(num="1700007", fileName=ExternalName_log , content=f"Errors encountered when the serial port is unavailable, disconnected, or already in use at runtime. The error is '{e}'", line=tb.tb_lineno)
            logProcess.receive(logName=logger_name, num="1700007", fileName=ExternalName_log, error=e)
            return 0
    ...
    def flushInputBuffer(self):
        try:
            ...
            # tb = notOpenErr.__traceback__  # 錯誤軌跡資訊
            # logProcess.centrailzedError(num="1700008", fileName=ExternalName_log, content="PortNotOpenError, Failed to clear the serial port buffer — the port was not open.", line=tb.tb_lineno)
            logProcess.receive(logName=logger_name, num="1700008", fileName=ExternalName_log, error=notOpenErr)
        pass
    ...
    def dump_fog_parameters(self, ch=2):
        try:
            ...
            logProcess.receive(logName=logger_name, num="", fileName=ExternalName_log, error=e)
            return False
            ...
            logProcess.receive(logName=logger_name, num="", fileName=ExternalName_log, error=jsonErr)
            return False
...
if __name__ == "__main__":
    ...
    conn = Connector()
    num, ports = conn.portList()
    print(f"目前共有 {num} 個 COM port")
    for p in ports:
        print(" -", p.device)  # 列出像 COM3, COM11 這種名稱
