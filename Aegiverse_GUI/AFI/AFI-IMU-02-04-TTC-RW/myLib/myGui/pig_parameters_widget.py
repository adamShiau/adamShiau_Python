...
class pig_parameters_widget(QGroupBox):
    def __init__(self, act=None , fileName="default_fog_parameters.json"):
        ...
        self.dump_bt.setFixedWidth(130)
        ...
        self.btnLayout.setContentsMargins(10, 15, 10, 15)
        self.btnLayout.addWidget(self.dump_bt)
        self.btnLayout.addWidget(self.loadTempFile)
        self.btnLayout.addWidget(self.updateBtn)

        # channel
        self.dropdown_para = comboGroup_1("channel")
        self.dropdown_para.addItem(["Z", "X", "Y"])   # Z -> 1, X -> 2, Y -> 3
    ...
    def initUI(self):
        ...
        # mainLayout.addWidget(self.KF_Q, 9, 0, 1, 2)
        # mainLayout.addWidget(self.KF_R, 9, 2, 1, 2)
        # mainLayout.addWidget(self.dataRate_sd, 10, 0, 1, 4)
        # mainLayout.addWidget(self.sfb, 10, 4, 1, 1)
        mainLayout.addWidget(self.cutoff, 8, 0, 1, 2)
        mainLayout.addWidget(self.Firmware_Version_lb, 9, 0, 1, 4)
        mainLayout.addWidget(self.GUI_Version_lb, 10, 0, 1, 4)
        ...
        mainLayout.addLayout(self.btnLayout, 0, 8, 1, 2)
        # mainLayout.addWidget(self.dump_bt, 0, 8, 1, 4)
        # mainLayout.addWidget(self.updateBtn, 1, 8, 1,4)
        # mainLayout.addWidget(self.loadTempFile, 2, 8, 1, 4)
    ...
    #     self.T1.le.editingFinished.connect(self.send_BIAS_T1_CMD)
    #     self.T2.le.editingFinished.connect(self.send_BIAS_T2_CMD)
    #     self.slope1.le.editingFinished.connect(self.send_SFB_SLOPE_1_CMD)
    #     self.slope2.le.editingFinished.connect(self.send_SFB_SLOPE_2_CMD)
    #     self.slope3.le.editingFinished.connect(self.send_SFB_SLOPE_3_CMD)
    #     self.offset1.le.editingFinished.connect(self.send_SFB_OFFSET_1_CMD)
    #     self.offset2.le.editingFinished.connect(self.send_SFB_OFFSET_2_CMD)
    #     self.offset3.le.editingFinished.connect(self.send_SFB_OFFSET_3_CMD)
    #
    #     self.limite_doublevalidator = QDoubleValidator()
    #     self.limite_doublevalidator.setNotation(QDoubleValidator.StandardNotation)
    #     self.limite_doublevalidator.setRange(-100000, 100000, 3)
    #
    #     self.dac_Vpi.le.editingFinished.connect(self.InputIntforEdit)
    #     self.dac_Vpi.le.textChanged.connect(self.isdoubleForDAC_Vpi)
    #     self.dac_Vpi.le.setValidator(self.limite_doublevalidator)
    #     self.dac_Vpi_status = False
    #
    #     self.rst_voltage.le.editingFinished.connect(self.InputIntforEdit)
    #     self.rst_voltage.le.textChanged.connect(self.isdoubleForRST_voltage)
    #     self.rst_voltage.le.setValidator(self.limite_doublevalidator)
    ...
    #     self.updateBtn.clicked.connect(self.updatelink)
    #     '''comboBox'''
    ...
    def linkfunction(self):
        ...
        self.loadTempFile.clicked.connect(self.loadTempandWriteBias)
        '''comboBox'''
        self.dropdown_para.cb.currentIndexChanged.connect(self.setChannel)
    ...
    def setChannel(self):
        ...
        # print(str(self.__chVal))
    # def record_change(self, func):  # 紀錄有需要更新的數值func
    #     funcNm = len(self.upFunc["Func"])
    #     if funcNm != 0:
    #         for i in range(funcNm):
    #             if func == self.upFunc["Func"][i]:  # 避免重複執行
    #                 print("pass")
    #                 pass
    #             else:
    #                 print(str(func))
    #                 self.upFunc["Func"]= np.append(self.upFunc["Func"], func)
    #     else:
    #         print(str(func))
    #         self.upFunc["Func"] = np.append(self.upFunc["Func"], func)
    #
    # def update_changevalue_loop(self):
    #     i = 0
    #     while i < len(self.upFunc["Func"]):
    #         t1 = time.perf_counter()
    #         self.updatelink(self.upFunc["Func"][i])
    ...
    def getVersion(self):
        ...
        self.GUI_Version_lb.setText('AFI-IMU-02-04-TTC-RW')

    def set_init_value(self, para):
        self.freq.spin.setValue(self.checkKeyExist(para, "FREQ"))
        self.wait_cnt.spin.setValue(self.checkKeyExist(para, "WAIT_CNT"))
        self.avg.spin.setValue(self.checkKeyExist(para, "ERR_AVG"))
        self.mod_H.spin.setValue(self.checkKeyExist(para, "MOD_H"))
        self.mod_L.spin.setValue(self.checkKeyExist(para, "MOD_L"))
        self.err_th.spin.setValue(self.checkKeyExist(para, "ERR_TH"))
        self.err_offset.spin.setValue(self.checkKeyExist(para, "ERR_OFFSET"))
        self.polarity.spin.setValue(self.checkKeyExist(para, "POLARITY"))
        self.const_step.spin.setValue(self.checkKeyExist(para, "CONST_STEP"))
        self.HD_Q.spin.setValue(self.checkKeyExist(para, "HD_Q"))
        self.HD_R.spin.setValue(self.checkKeyExist(para, "HD_R"))
        self.gain1.spin.setValue(self.checkKeyExist(para, "GAIN1"))
        self.gain2.spin.setValue(self.checkKeyExist(para, "GAIN2"))
        self.cutoff.spin.setValue(self.checkKeyExist(para, "CUTOFF"))
        ...
        sf0_val = (self.checkKeyExist(para, "SF0") * 10000)
        sf1_val = (self.checkKeyExist(para, "SF1") * 10000)
        ...
        ## 20240417 修改
        # #將會有可能產生科學符號的部份，使用此方式將科學符號轉為標準表示法
        slope1_process = self.checkKeyExist(para, "SFB_1_SLOPE")
        slope2_process = self.checkKeyExist(para, "SFB_2_SLOPE")
        slope3_process = self.checkKeyExist(para, "SFB_3_SLOPE")
        offset1_process = self.checkKeyExist(para, "SFB_1_OFFSET")
        offset2_process = self.checkKeyExist(para, "SFB_2_OFFSET")
        offset3_process = self.checkKeyExist(para, "SFB_3_OFFSET")

        self.T1.le.setText(f'{para["BIAS_COMP_T1"]:.1f}'.rstrip('0').rstrip('.'))
        self.T2.le.setText(f'{para["BIAS_COMP_T2"]:.1f}'.rstrip('0').rstrip('.'))
        ...
        if filename != "":
            ...
            if isinstance(tempVal, pd.Series) and isinstance(slopeVal, pd.Series) and isinstance(offsetVal, pd.Series):
                try:
                    ...
                    self.slope2.le.setText(f'{slopeVal[2]:.10f}'.rstrip('0').rstrip('.'))
    ...
    #     # logger.info('set DAC gain: %d', value)
    #     self.__act.writeImuCmd(CMD_FOG_TMIN, value2[0], self.__chVal)
    #     # self.__par_manager.update_parameters("DAC_GAIN", value)
    #     print("%x" % value2[0])
    #
    ...
    #     value2 = struct.unpack('<I', struct.pack('<f', value))
    #     # logger.info('set DAC gain: %d', value)
    #     self.__act.writeImuCmd(CMD_FOG_TMAX, value2[0], self.__chVal)
    #     # self.__par_manager.update_parameters("DAC_GAIN", value)
    #     print("%x" % value2[0])
    ...
    def send_CUTOFF_CMD(self):
        ...
        # value2 = struct.unpack('<I', struct.pack('<f', value))
        # logger.info('set DAC gain: %d', value)
        self.__act.writeImuCmd(CMD_FOG_CUTOFF, value, self.__chVal)
        # self.__par_manager.update_parameters("DAC_GAIN", value)
        # print("%d" % value)
    ...
    def send_SFB_CMD(self):
        ...
        self.__act.writeImuCmd(CMD_FOG_SFB, value[0], self.__chVal)
    ...
    def update_FPGA_Q(self):
        ...
        self.__act.writeImuCmd(CMD_FOG_FPGA_Q, value, self.__chVal)
    ...
    def update_FPGA_R(self):
        ...
        self.__act.writeImuCmd(CMD_FOG_FPGA_R, value, self.__chVal)
    ...
    def send_DATA_RATE_CMD(self):
        ...
        self.__act.writeImuCmd(CMD_FOG_INT_DELAY, value, self.__chVal)
