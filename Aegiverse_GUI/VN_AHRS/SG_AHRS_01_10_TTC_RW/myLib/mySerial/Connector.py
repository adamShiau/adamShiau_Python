...
class Connector:
    ...
    def send_vnwr_command(self, command_str: str = "$VNWRG,75,1,10,05,0129,0010*66"):
        """
        傳送自訂的 VNWRG 指令字串，例如:
        $VNWRG,75,1,10,05,0129,0010*66

        Args:
            command_str (str): 要傳送的完整指令字串（含開頭 $ 與結尾 *checksum）
        """
        try:
            if not self.__ser.is_open:
                logger.error("Port not open. Please connect first.")
                return False

            # 加上 CRLF 結尾
            cmd = (command_str + "\r\n").encode('utf-8')
            self.__ser.write(cmd)
            logger.info(f"Send command: {command_str}")
            return True

        except serial.SerialException as e:
            __excType, __excObj, __excTb = sys.exc_info()
            __lineNum = __excTb.tb_lineno
            logger.error(f"Send VNWRG command failed: {e}, line {__lineNum}")
            return False
...
if __name__ == "__main__":
    ...
    print("[1] Send: $VNRST*4D")
    ser.send_vnwr_command("$VNRST*4D")

    time.sleep(0.1)

    # 2️⃣ Send configuration command
    print("[2] Send: $VNWRG,75,1,10,05,0129,0010*66")
    ser.send_vnwr_command("$VNWRG,75,1,10,05,0129,0010*66")

    print("[3] Start continuous reading (Ctrl+C to stop)...\n")

    try:
        while True:
            # 讀取目前緩衝區中所有資料
            available = ser.readInputBuffer()
            if available > 0:
