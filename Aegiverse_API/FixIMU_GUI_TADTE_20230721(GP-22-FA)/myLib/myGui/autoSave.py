...
class atSave_PC_v3(QObject):
    ...
    def __init__(self, fnum=0):
        ...
        self.yy = None
        self.__data_manager = cmn.data_manager(fnum=fnum)
        self.__Temp_manager = cmn.data_manager(fnum=1)
        self.hh_path = None
        # self.data_path = rootPath + '/' + 'data'
        self.__data_path = None
        self.dd_path = None
    ...
    def creating_Folder(self,file_type, __Time, T_path_num):  # 0605 修改重複的code
        __time = {0:self.data_path, 1:self.yy_path, 2: self.mm_path, 3: self.dd_path, 4: self.PDTemp_path}
    ...
    def create_PD_temperature_folder(self):  # 20230627 建立溫度資料夾
        self.PDTemp_path = self.dd_path + '/Temperature'
        self.creating_Folder("Temp", "Temperature", 4)
    ...
    def auto_create_folder(self, en=False):
        if en:
            ...
            self.create_month_folder()
            self.create_day_folder()
            self.create_PD_temperature_folder()
    ...
    def open_hour_file(self):
        ...
        if not file_exist:
            ...
            self.create_Temperature_file(file_current_time)
        self.hh_reg = self.hh
    ...
    def create_data_file(self, time):
        ...
        self.write_line(0, '  time,      wx,      wy,      wz,      ax,      ay,      az,  T')
        print('     create hour file %s done.' % self.__data_manager.name)

    def create_Temperature_file(self, time):
        filename = self.PDTemp_path + '/' + "PD_" + time
        self.PD_temp_path = filename + '.txt'
        self.__Temp_manager.name = self.PD_temp_path
        self.__Temp_manager.open(True)
        self.write_line(1, 'PC_time, Temperature_SP9, Temperature_SP13, Temperature_SP14')
        print('     create hour file %s done.' % self.__Temp_manager.name)

    def close_hour_folder(self):
        self.__data_manager.close()
        print('     close hour file %s done.' % self.__data_manager.name)
        self.__Temp_manager.close()
        print('     close hour file %s done.' % self.__Temp_manager.name)
        pass
    ...
    def write_line(self, number, comment):   # 呼叫寫進txt檔案的method
        if number == 0:
            ...
            self.__Temp_manager.write_line(comment)

    def saveData(self, number, datalist, fmt):
        if number == 0:
            self.__data_manager.saveData(datalist, fmt)
        elif number == 1:
            self.__Temp_manager.saveTemp(datalist)
