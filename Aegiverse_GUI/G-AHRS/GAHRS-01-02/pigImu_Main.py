...
class mainWindow(QMainWindow):
    ...
    def __init__(self, debug_en: bool = False):
        ...
        self.gpsdata_file = cmn.data_manager(fnum=1)  # GPS數據文件管理器
    ...
    def linkfunction(self):
        ...
        self.act.gps_data_qt.connect(self.handleGpsData)
    ...
    def start(self):
        ...
        if self.top.save_block.rb.isChecked():
            ...
            if self.top.save_block.rcs_cb.isChecked():
                ...
                gps_file_name = self.top.save_block.le_filename.text() + '_Rcs_GPS' + self.top.save_block.le_ext.text()
            else :
                file_name = self.top.save_block.le_filename.text() + self.top.save_block.le_ext.text()
                gps_file_name = self.top.save_block.le_filename.text() + '_GPS' + self.top.save_block.le_ext.text()
            ...
            self.imudata_file.write_line('time,wx,wy,wz,ax,ay,az,T,pitch,roll,yaw,gps_status_code')

            # 開啟GPS數據文件
            self.gpsdata_file.name = gps_file_name
            self.gpsdata_file.open(self.top.save_block.rb.isChecked())
            self.gpsdata_file.write_line('mcu_time,latitude,longitude,altitude,utc_year,utc_month,utc_day,utc_hour,utc_minute,utc_second,utc_millisecond,gps_status_code')
    ...
    def stop(self):
        ...
        if self.top.save_block.rb.isChecked():
            ...
            self.gpsdata_file.close()  # 關閉GPS數據文件
    ...
    def handleGpsData(self, gps_data):
        """處理GPS數據的獨立方法"""
        try:
            if not gps_data:
                return
            ...
            # 更新GNSS狀態顯示 (如果有狀態信息)
            if 'GPS_STATUS_CODE' in gps_data and 'GPS_STATUS_NAME' in gps_data:
                self.top.updateGnssStatus(gps_data['GPS_STATUS_CODE'], gps_data['GPS_STATUS_NAME'])

            # 更新GPS詳細信息視窗
            self.top.updateGpsDetails(gps_data)

            # 保存GPS數據到檔案 (如果有開啟保存)
            if self.top.save_block.rb.isChecked() and hasattr(self, 'gpsdata_file'):

                # GPS數據列表 - mcu_time在第一位，移除重複的timestamp
                gps_datalist = [
                    gps_data.get('MCU_TIME', 0),            # mcu_time (第一欄)
                    gps_data.get('LATITUDE', 0),            # latitude
                    gps_data.get('LONGITUDE', 0),           # longitude
                    gps_data.get('ALTITUDE', 0),            # altitude
                    gps_data.get('UTC_YEAR', 0),            # utc_year
                    gps_data.get('UTC_MONTH', 0),           # utc_month
                    gps_data.get('UTC_DAY', 0),             # utc_day
                    gps_data.get('UTC_HOUR', 0),            # utc_hour
                    gps_data.get('UTC_MINUTE', 0),          # utc_minute
                    gps_data.get('UTC_SECOND', 0),          # utc_second
                    gps_data.get('UTC_MILLISECOND', 0),     # utc_millisecond
                    gps_data.get('GPS_STATUS_CODE', 0)      # gps_status_code
                ]

                # 格式化字串 - mcu_time在第一位，移除timestamp
                gps_data_fmt = "%.3f,%.6f,%.6f,%.2f,%d,%d,%d,%d,%d,%d,%d,%d"
                self.gpsdata_file.saveData(gps_datalist, gps_data_fmt)

        except Exception as e:
            print(f"Error handling GPS data: {e}")

    def collectData(self, imudata):  #　1000002
        if not self.press_stop:
            # 檢查數據是否為空或無效
            if imudata is None:
                logger.warning('1000002, Received None data from Reader, skipping frame')
                return
            ...
            try:
                ...
                # print(imudata["PIG_WZ"])
                # 取得 GPS 狀態碼，如果沒有就設為 255 (INVALID)
                gps_status = imudata.get("GPS_STATUS_CODE", [255])

                datalist = [imudata["TIME"], imudata["WX"], imudata["WY"], imudata["WZ"],
                            imudata["AX"], imudata["AY"], imudata["AZ"], imudata["PD_TEMP"],
                            imudata["PITCH"], imudata["ROLL"], imudata["YAW"], gps_status]
                data_fmt = "%.3f,%.5f,%.5f,%.5f,%.5f,%.5f,%.5f,%.1f,%.5f,%.5f,%.5f,%d"
                ...
                __excType, __excObj, __excTb = sys.exc_info()
                __lineNum = __excTb.tb_lineno
                logger.warning(f'1000002, {type(e).__name__} - Data corrupted, skipping this frame, line {__lineNum}. Details: {e}')
                # 跳過這筆損壞的數據，繼續處理下一筆
                return
            except Exception as e:
                __excType, __excObj, __excTb = sys.exc_info()
                __lineNum = __excTb.tb_lineno
                logger.error(f'1000002, {type(e).__name__} - Critical error in data collection, line {__lineNum}. Details: {e}')
                # 對於其他嚴重錯誤，仍然調用 occurError
                self.occurError(errStr="撈取數據功能出現錯誤。")
