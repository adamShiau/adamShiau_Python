...
import QSS011_Widget_2 as UI
import QSS011_Action_2 as ACT
import numpy as np
...
class mainWindow(QMainWindow):
	def __init__(self, parent = None):
		...
		self.act = ACT.qss011Act(self.loggername)
	...
	def linkFunction(self):
		...
		self.top.open.clicked.connect(self.set_mode)
		self.top.close.clicked.connect(self.set_mode)
...
#Connection
	...
	def usbConnect(self):
		...
		if self.usbConnStatus:
			...
			self.top.enableButtomsetting(True)
		else:
			self.top.usb.SetConnectText(Qt.red,"Connect failed", True)

	def set_mode(self, firstConnect = False):
		if self.act.runFlag or self.loadUIpreset:
			return
	...
	def buttonStart(self):
		...
		self.top.enableButtomsetting(False)
	...
	def plotFog(self, data, dt):
		...
		self.top.plot.ax.set_xlabel("dT (s)")
		self.top.plot.ax.set_ylabel("")
		self.top.plot.ax.plot(self.dt, self.data, color = 'blue', linestyle = '-', marker = '*')
		self.top.plot.figure.canvas.draw()
		self.top.plot.figure.canvas.flush_events()
