...
def file_manager(isopen=False, name="notitle", mode="w", fnum=0):
    ...
    if isopen:
        ...
        try:
            ...
            # logProcess.centrailzedError(num="1500001", fileName=ExternalName_log, content="NameError", line=tb.tb_lineno)
            logProcess.receive(logName=logger_name, num="1500001", fileName=ExternalName_log, error=e)
            pass
        except AttributeError as e:
            # print("file_manager: AttributeError")
            tb = e.__traceback__  # 錯誤軌跡資訊
            logProcess.centrailzedError(logName=logger_name, num="1500001", fileName=ExternalName_log, content='AttributeError - the file attempt to close does not exist!', line=tb.tb_lineno)
...
def loadCSVFile(filename):  # 1500002
    try:
        ...
        # tb = e.__traceback__  # 錯誤軌跡資訊
        # logProcess.centrailzedError(num="", fileName=ExternalName_log, content=f"The format of the loaded file is incorrect. Please verify the content format.", line=tb.tb_lineno)
        logProcess.receive(logName=logger_name, num="1500002", fileName=ExternalName_log, error=e)
        return None
    except pd.errors.EmptyDataError as e:
        # tb = e.__traceback__  # 錯誤軌跡資訊
        # logProcess.centrailzedError(num="", fileName=ExternalName_log, content=f"The content of the file is empty or contains only the header row.", line=tb.tb_lineno) # 載入檔案的內文為空白或是只有標題行。
        logProcess.receive(logName=logger_name, num="1500002", fileName=ExternalName_log, error=e)
        return None
...
def IEEE_754_INT2F(datain):
    if len(datain) == 4:
        ...
        try:
            ...
            # tb = e.__traceback__  # 錯誤軌跡資訊
            # logProcess.centrailzedError(num="", fileName=ExternalName_log, content="An error occurred while converting data using struct.-1", line=tb.tb_lineno)
...
def IEEE_754_INT2F_R(datain):
    if len(datain) == 4:
        ...
        try:
            ...
            # tb = e.__traceback__  # 錯誤軌跡資訊
            # logProcess.centrailzedError(num="", fileName=ExternalName_log, content="An error occurred while converting data using struct.-2", line=tb.tb_lineno)
            logProcess.receive(logName=logger_name, num="", fileName=ExternalName_log, error=e)
            return -1
        # print('%.3f' % f[0])
        return f[0]
