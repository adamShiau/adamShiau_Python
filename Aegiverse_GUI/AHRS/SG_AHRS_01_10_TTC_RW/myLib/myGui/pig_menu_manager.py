...
class pig_menu_manager:
    ...
    def action_list(self, obj):
        self.pig_events_log_action = QAction("Events View", obj)
        self.file_menu.addAction(self.pig_events_log_action)
        ...
        self.pig_W_A_cali_action = QAction("pig calibration parameters", obj)
        self.pig_W_A_cali_action.setShortcut("Ctrl+m")
        self.pig_W_A_cali_action.setEnabled(False)
        self.fog_select_cmd_action = QAction("fog choice command", obj)
        self.fog_select_cmd_action.setEnabled(False)
        ...
        self.setting_menu.addAction(self.fog_select_cmd_action)
        ...
        self.pig_kal_filter = QAction("setting Kalman")
        self.pig_kal_filter.setDisabled(True)
        self.setting_menu.addAction(self.pig_kal_filter)

    def action_trigger_connect(self, fn):
        self.pig_events_log_action.triggered.connect(fn[0])
        self.pig_para_action.triggered.connect(fn[1])
        self.pig_cali_action.triggered.connect(fn[2])
        self.pig_plot_action.triggered.connect(fn[3])
        self.pig_allan_action.triggered.connect(fn[4])
        self.pig_initial_setting_action.triggered.connect(fn[5])
        self.pig_W_A_cali_action.triggered.connect(fn[6])
        self.fog_select_cmd_action.triggered.connect(fn[7])
        self.pig_kal_filter.triggered.connect(fn[8])
    ...
    def setEnable(self, open):
        ...
        self.fog_select_cmd_action.setEnabled(open)